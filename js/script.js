...
async function lerComprovante() {
  ...
  try {
    ...
    if (resultado.valor !== null && resultado.valor !== undefined) {
      ...
      if (resultado.descricao) document.getElementById("descricao").value = resultado.descricao;
      if (resultado.categoria) {
        const sel = document.getElementById("categoria");
        const opcoes = Array.from(sel.options).map(o => o.value);
        if (opcoes.includes(resultado.categoria)) sel.value = resultado.categoria;
      }
